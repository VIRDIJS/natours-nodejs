...
var displayMap = function displayMap(locations) {
  ...
  locations.forEach(function (loc) {
    ...
    new mapboxgl.Popup({
      offset: 30
    }).setLngLat(loc.coordinates).setHTML("<p>".concat(loc.day, ": ").concat(loc.description, "</p>")).addTo(map); // Extend the bounds to include the current location
    ...
    return regeneratorRuntime.wrap(function _callee$(_context) {
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            _context.prev = 0;
            stripe = Stripe('pk_test_51HKEneGyAbOwMTQYmQZ9dToVkQeHaE9fRcxLwYX1dqP9j6OPfToO3mqKYG7wIkWEdYlUdxRWz7sN6e3KIbfdbfuj00iBZ61GEj'); // 1. Get checkout session from API
      ...
      return regeneratorRuntime.wrap(function _callee$(_context) {
        while (1) {
          switch (_context.prev = _context.next) {
            ...
            case 7:
              document.querySelector('.btn--save-password').textContent = 'Save password'; // Delete the content of the fields once form is submitted
...
if (bookBtn) {
  bookBtn.addEventListener('click', function (e) {
    e.target.textContent = 'Processing...'; // Using Destructuring
    // const { tourId } = e.target.dataset 
    // OR the following syntax

    var tourId = e.target.dataset.tourId; // Call bookTour(tourId) in stripe.js 
